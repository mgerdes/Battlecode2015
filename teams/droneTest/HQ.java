...
import droneTest.constants.Building;
import droneTest.communication.Channel;
import droneTest.constants.Order;
import droneTest.constants.Symmetry;
...
public class HQ {
    ...
    private static void updateSpawningAndOrders() throws GameActionException {
        ...
        //--Spawn drones until round 450
        int currentRound = Clock.getRoundNum();
        if (currentRound < 500) {
            HqOrders.setSpawn(RobotType.DRONE, SPAWN_ON);
        }
        else {
            HqOrders.setSpawn(RobotType.DRONE, SPAWN_OFF);
        }

        HqOrders.setDefaultFor(RobotType.DRONE, Order.AttackEnemyStructure);
